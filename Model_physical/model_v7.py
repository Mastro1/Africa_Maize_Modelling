"""
MODEL V7 – Phased Stress Crop Yield Model (V3.4 Dynamic Calendar)
================================================================
DIFFERENCES FROM V5:
...
class MaizeYieldModelV7:
    def __init__(self, data_dir, gadm_data_dir, fpar_file, era5_new_file,
        ...
        # --- Model Parameters (Initialized with Defaults) ---
        self.T_OPT = T_OPT
        self.T_SIGMA = T_SIGMA
        self.VEGETATIVE_WEIGHT = VEGETATIVE_WEIGHT
        self.REPRODUCTIVE_WEIGHT = REPRODUCTIVE_WEIGHT
        self.CRITICAL_WINDOW_DAYS = CRITICAL_WINDOW_DAYS
        self.HEAT_THRESHOLD = HEAT_THRESHOLD
        self.HEAT_PENALTY = HEAT_PENALTY
        self.DROUGHT_WS_THRESHOLD = DROUGHT_WS_THRESHOLD
        self.DROUGHT_PENALTY = DROUGHT_PENALTY
    ...
    def load_optimized_parameters(self, metric='correlation'):
        """
        Looks for optimized parameters CSV in Model_physical/Results/V7_model_optimized/
        If found, replaces the default parameters with those found by Optuna.
        """
        opt_dir = os.path.join(os.path.dirname(self.output_dir), "V7_model_optimized")
        opt_file = os.path.join(opt_dir, f"Optimal_Params_Optuna_{self.country}_{metric}.csv")
        ...
        if os.path.exists(opt_file):
            ...
            if not df_opt.empty:
                ...
                mapping = {
                    'T_OPT': 'T_OPT',
                    'VEGETATIVE_WEIGHT': 'VEGETATIVE_WEIGHT',
                ...
                for col, attr in mapping.items():
                    if col in df_opt.columns:
                        val = df_opt.iloc[0][col]
                        setattr(self, attr, val)
                        print(f"  - {attr}: {val}")
            print(f"--------------------------------------\n")
        else:
            print(f"--- [Using Default Parameters] ---")
            print(f"No optimized parameters file found for {self.country}.\n")
    ...
    def load_and_merge_data(self):
        ...
        # NDVI STSG — no longer needed in df_daily for V7 (calendar uses prepare_calendar_data)
    ...
    def prepare_calendar_data(self):
        ...
        # We can derive this relative to the output_dir which is Model_physical/Results/V7_model
    ...
    # ------------------------------------------------------------------
    # YIELD CALCULATION  (V7 – Phased + Critical Window)
    # ------------------------------------------------------------------
    def calculate_yield_estimates(self, df_daily, df_calendar, pcode_cal_dict):
        print("Calculating Yield estimates (V7 – Phased Stress)...")

        results = []
        # Filter PCODEs: Must be in both calendar and crop area (filtered by country)
        df_cal_pcodes = df_calendar['PCODE'].unique()
        if self.crop_area_path and os.path.exists(self.crop_area_path):
            df_area = pd.read_csv(self.crop_area_path)
            # Filter crop area by country first
            ...
            print(f"  Filtering locations: {len(df_cal_pcodes)} in calendar, {len(crop_area_pcodes)} in crop area for {self.country}.")
            print(f"  Valid locations (intersection): {len(valid_pcodes)}")
        else:
            print("  WARNING: Crop area file not found or path not provided. Filtering only by calendar.")
            valid_pcodes = set(df_cal_pcodes)

        pcodes = [p for p in df_daily['PCODE'].unique() if p in valid_pcodes]
        total = len(pcodes)
        print(f"  PCODEs to process (after data intersection): {total}")
        ...
        for count, pcode in enumerate(pcodes, 1):
            ...
            # V7: Try to retrieve dynamic dates, but fallback is allowed later
            ...
            for year in years:
                # ------------------------------------------------
                # V7 LOGIC: Load pre-calculated V3.4 dates (Dual Seasons supported)
                # ------------------------------------------------
                ...
                for season_idx, dates in seasons_to_process.items():
                    ...
                    # Stress factors
                    # ------------------------------------------------
                    df_season['Ts'] = np.exp(-(df_season['Temp_C'] - self.T_OPT)**2 / (2 * self.T_SIGMA**2))
                    
                    # f(VPD): Vapor Pressure Deficit Stress
                    df_season['f_VPD'] = 1.5 / (1.5 + df_season['VPD_kPa'])
                    
                    # Ws: Water stress based on VPD
                    df_season['Ws'] = df_season['f_VPD']
                    
                    min_stress = np.minimum(df_season['Ts'], df_season['Ws'])

                    # ------------------------------------------------
                    # V7: Phased NPP Accumulation
                    # ------------------------------------------------
                    df_season['NPP_daily'] = (
                        df_season['PAR_MJ'] *
                    ...
                    if silking_date is not None and start_date <= silking_date <= end_date:
                        ...
                        npp_total = (
                            (df_season.loc[mask_veg, 'NPP_daily'] * self.VEGETATIVE_WEIGHT).sum() +
                            (df_season.loc[mask_rep, 'NPP_daily'] * self.REPRODUCTIVE_WEIGHT).sum()
                        )
                    else:
                        # No silking date → flat weighting (average of the two weights)
                        avg_weight = (self.VEGETATIVE_WEIGHT + self.REPRODUCTIVE_WEIGHT) / 2.0
                        npp_total = (df_season['NPP_daily'] * avg_weight).sum()

                    # ------------------------------------------------
                    # V7: Critical Window Penalty (around Silking)
                    # ------------------------------------------------
                    heat_penalty_applied = 1.0
                    drought_penalty_applied = 1.0

                    if silking_date is not None and start_date <= silking_date <= end_date:
                        cw_start = silking_date - pd.Timedelta(days=self.CRITICAL_WINDOW_DAYS)
                        cw_end   = silking_date + pd.Timedelta(days=self.CRITICAL_WINDOW_DAYS)
                        ...
                        if not df_cw.empty:
                            ...
                            if avg_temp_cw > self.HEAT_THRESHOLD:
                                heat_penalty_applied = self.HEAT_PENALTY
                            if avg_ws_cw < self.DROUGHT_WS_THRESHOLD:
                                drought_penalty_applied = self.DROUGHT_PENALTY

                    # ------------------------------------------------
                    # Final yield conversion
                    # ------------------------------------------------
                    biomass = npp_total / self.C_FRAC
                    partitioning = self.HI / ((1 + self.RS) * (1 - self.MC))
    ...
    def save_and_merge_results(self, df_new):
        """
        Saves country results and merges them into the Global V7 CSV.
        Replaces previous records for the same country to avoid duplicates.
        """
        # Save country-specific file
        country_out = os.path.join(self.output_dir, f"maize_yield_estimates_V7_{self.country.replace(' ', '_')}.csv")
        df_new.to_csv(country_out, index=False)
        print(f"\nCountry results saved to: {country_out}")

        # Global Merge (Replace on Append)
        global_out = os.path.join(self.output_dir, "Global_Maize_Yield_V7.csv")
    ...
    def plot_example(self, df_daily, df_results, pcode):
        ...
        plot_path = os.path.join(self.output_dir, f"{pcode}_timeseries_V7.png")
        ...
        axes[0].plot(daily_subset['date'], daily_subset['FPAR_smooth'], '-',
                     label='STSG FPAR (V7)', linewidth=1.5, color='blue')
        ...
        # Plot 2: Stress
        ts = np.exp(-(daily_subset['Temp_C'] - self.T_OPT)**2 / (2 * self.T_SIGMA**2))
        f_vpd = 1.5 / (1.5 + daily_subset['VPD_kPa'])
        
        # WP and FC were missing in local scope? Actually they aren't defined in Model_v7 global
        # but referenced in previous versions. In V7, SM stress seems to be removed or simplified.
        # Let's check the Stress plot logic in V7.
        
        axes[1].plot(daily_subset['date'], ts, label='Temp Stress (Ts)', alpha=0.7)
        axes[1].plot(daily_subset['date'], f_vpd, '--', label='VPD Stress (f_VPD)', alpha=0.5)
        # Deleted soil moisture stress plot if not used
        axes[1].plot(daily_subset['date'], f_vpd, label='Water Stress (Ws)', color='black', linewidth=1.5)
        ...
        if not res_subset.empty:
            ...
            axes[2].bar(res_dates, res_subset['Yield_Estimated_t_ha'], width=60,
                        label='Yield V7 (t/ha)', color='green', alpha=0.6)
...
def run_model_V7_for_country(country, target_pcode=None):
    """
    Modular entry point to run Model V7 for a specific country.
    """
    ...
    VI_FILE = os.path.join(GADM_DATA_DIR,
                           f"{country.replace(' ', '_').replace("'", "_")}_admin2_VI_timeseries_GADM.csv")

    fPAR_FILE = f"{country.replace(' ', '_').replace("'","_")}_admin2_FPAR_timeseries_GLAD.csv"
    if not os.path.exists(os.path.join(DATA_DIR, fPAR_FILE)):
        print("Admin 2 data for fPAR not existing for admin2. Trying with admin 1 instead")
        fPAR_FILE = f"{country.replace(' ', '_').replace("'","_")}_admin1_FPAR_timeseries_GLAD.csv"

    ERA5_NEW_FILE = f"{country.replace(' ', '_').replace("'","_")}_admin2_new_ERA5_timeseries.csv"
    if not os.path.exists(os.path.join(DATA_DIR, ERA5_NEW_FILE)):
        print("Admin 2 data for ERA5_NEW not existing for admin2. Trying with admin 1 instead")
        ERA5_NEW_FILE = f"{country.replace(' ', '_').replace("'","_")}_admin1_new_ERA5_timeseries.csv"

    ERA5_GADM_FILE = f"{country.replace(' ', '_').replace("'","_")}_admin2_ERA5_timeseries_GADM.csv"
    if not os.path.exists(os.path.join(GADM_DATA_DIR, ERA5_GADM_FILE)):
        print("Admin 2 data for ERA5_GADM not existing for admin2. Trying with admin 1 instead")
        ERA5_GADM_FILE = f"{country.replace(' ', '_').replace("'","_")}_admin1_ERA5_timeseries_GADM.csv"

    print(f"\n{'='*60}")
    print(f" RUNNING MODEL V7: {country}")
    print(f"{'='*60}")
    ...
    model = MaizeYieldModelV7(
        ...
        fpar_file=fPAR_FILE,
        era5_new_file=ERA5_NEW_FILE,
        era5_gadm_file=ERA5_GADM_FILE,
    ...
    # -----------------------------------------------------------------
    import verify_fao
    print(f"\nRunning FAO Verification (V7) for {country}...")
    verify_fao.verify_fao(country_name=country,
                          input_results_dir=OUTPUT_DIR,
                          version='V7')

    import verify_fao_dual_season_relative
    print(f"\nRunning FAO Dual Season Relative Verification (V7) for {country}...")
    verify_fao_dual_season_relative.verify_fao(country_name=country,
                                                                    input_results_dir=OUTPUT_DIR,
                                                                    version='V7')

    import verify_hsa
    print(f"\nRunning HarvestStat Africa Verification (V7) for {country}...")
    #verify_hsa.verify_hsa(country_name=country,
                          #input_results_dir=OUTPUT_DIR,
                          #version='V7')

    print(f"\nGenerating Yield Anomaly Maps for {country}...")
    #visualize_yield_maps(country=country, version='V7')

    print(f"\nRunning Stress Visualization (V7) for {country}...")
    visualize_stress_v7.run_stress_visualization(country, target_pcode=target_pcode, model_instance=model)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run Maize Yield Model V7 (Dual Seasons)")
    ...
    if args.country == "All":
        ...
        modeled_countries = pd.read_csv(r"Model_physical\Results\V7_model\Global_Maize_Yield_V7.csv")
        modeled_countries = modeled_countries["Country"].unique().tolist()
        modeled_countries = [c.replace('_', ' ') for c in modeled_countries]
        print(modeled_countries)

        COUNTRY_LIST = countries
        missing = [c for c in COUNTRY_LIST if c not in modeled_countries]
        print("missing countries:", missing)
        
        for country in missing:
            try:
                run_model_V7_for_country(country, target_pcode=None)
            except Exception as e:
        ...
        run_model_V7_for_country(args.country, args.pcode)
