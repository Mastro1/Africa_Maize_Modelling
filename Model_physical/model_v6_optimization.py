...
# --- Stress Factor ---
# T_OPT is now a parameter passed to the MaizeYieldModelV6 class
...
class MaizeYieldModelV6:
    def __init__(self, data_dir, gadm_data_dir, fpar_file, era5_new_file,
                 ...
                 country, vi_file, crop_area_file=None, t_opt=25.0):
        """
        Parameters
        ----------
        country : str
        ...
        t_opt : float
            Optimal temperature (°C) to be tested for this run.
        """
        self.t_opt = t_opt
        ...
        for count, pcode in enumerate(pcodes, 1):
            ...
            for year in years:
                ...
                for season_idx, dates in seasons_to_process.items():
                    ...
                    # Stress factors
                    # ------------------------------------------------
                    df_season['Ts'] = np.exp(-(df_season['Temp_C'] - self.t_opt)**2 / (2 * T_SIGMA**2))
    ...
    # MAIN RUNNER
    # ------------------------------------------------------------------
    def run_full_model(self, country, pcode=None, save_results=True):
    ...
    def plot_example(self, df_daily, df_results, pcode):
        ...
        # Plot 2: Stress
        ts = np.exp(-(daily_subset['Temp_C'] - self.t_opt)**2 / (2 * T_SIGMA**2))
...
def run_model_v6_for_country(country, target_pcode=None, t_opt_val=25.0, save_results=True):
    """
    ...
    print(f"\n{'='*60}")
    print(f" RUNNING MODEL V6: {country} [T_OPT={t_opt_val}°C]")
    print(f"{'='*60}")
    ...
    df_results = model.run_full_model(country=country, pcode=target_pcode, save_results=save_results)
...
def optimize_country_temperature(country, target_pcode=None, bounds=(20, 32)):
    """
    Search for the optimal T_OPT that maximizes FAO Kernel Detrended Correlation.
    """
    import concurrent.futures
    import multiprocessing
    
    best_t_opt = None
    best_corr_ker = -np.inf
    
    BASE_DIR = r"c:\Users\MassimoPoretti\Documents\Python Projects\Africa_Maize_Modelling"
    OUTPUT_DIR = os.path.join(BASE_DIR, "Model_physical", "Results", "V6_model")
    optimization_results_path = os.path.join(OUTPUT_DIR, "T_OPT_Optimization_Results.csv")
    
    avail_cores = max(1, multiprocessing.cpu_count() - 1)
    print(f"Starting T_OPT Optimization for {country} (Bounds: {bounds[0]}-{bounds[1]}°C) sequentially using multiprocessing...")
    
    # -----------------------------------------------------------------
    # STEP 0: Sequential Warm-up (PREVENTS RACE CONDITIONS)
    # -----------------------------------------------------------------
    # Running the model once sequentially ensures that any missing data 
    # (like STSG files or Dynamic Calendar caches) are generated before 
    # multiple parallel workers try to do it simultaneously.
    print(f"  - Preparing data caches for {country} (sequential warm-up)...")
    run_model_v6_for_country(country, target_pcode, t_opt_val=25.0, save_results=False)
    print(f"  - Caches ready. Launching parallel evaluating pool ({avail_cores} cores)...\n")
    
    args_list = [(t, country, target_pcode) for t in range(bounds[0], bounds[1] + 1)]
    
    # Use max_workers avoiding full CPU block, leaving a core free
    
    with concurrent.futures.ProcessPoolExecutor(max_workers=avail_cores) as executor:
        results = executor.map(_eval_t_opt_wrapper, args_list)
        
        for t_val, corr_ker in results:
            print(f"Evaluated T_OPT: {t_val}°C -> Kernel Corr: {corr_ker}")
            if corr_ker is not None and not np.isnan(corr_ker) and corr_ker > best_corr_ker:
                best_corr_ker = corr_ker
                best_t_opt = float(t_val)
            
    # Fallback to standard 25 if optimization failed
    if best_t_opt is None:
        print(f"Optimization failed to find a valid correlation for {country}. Defaulting to T_OPT=25.0°C.")
        best_t_opt = 25.0
        best_corr_ker = 0.0
    
    print(f"\n{'='*60}")
    print(f" OPTIMIZATION COMPLETE FOR {country}")
    print(f" Best T_OPT: {best_t_opt}°C (Kernel Corr: {best_corr_ker:.4f})")
    print(f"{'='*60}\n")
    
    # Run one final time with Best T_OPT and save_results=True
    run_model_v6_for_country(country, target_pcode, t_opt_val=best_t_opt, save_results=True)
    
    # Save optimized parameter to CSV
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    if os.path.exists(optimization_results_path):
        df_opt = pd.read_csv(optimization_results_path)
        # Remove old entry if exists
        df_opt = df_opt[df_opt['Country'] != country]
    else:
        df_opt = pd.DataFrame(columns=['Country', 'Optimal_T_OPT', 'Kernel_Correlation'])
        
    new_entry = pd.DataFrame([{
        'Country': country,
        'Optimal_T_OPT': best_t_opt,
        'Kernel_Correlation': best_corr_ker
    }])
    df_opt = pd.concat([df_opt, new_entry], ignore_index=True)
    df_opt.to_csv(optimization_results_path, index=False)
    print(f"Updated Optimization Results File: {optimization_results_path}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Optimize Maize Yield Model V6 (Dual Seasons)")
    ...
    if args.country != "all":
        optimize_country_temperature(args.country, target_pcode=args.pcode)
    else:
        print("Starting T_OPT Optimization for all countries...")
        COUNTRIES = ['Uganda', 'Tanzania', 'Rwanda', 'Burundi', 'Mozambique', 'Zimbabwe', 'Somalia', 'South Sudan', 'Sudan', 'Namibia', 'Botswana', 'Democratic Republic of the Congo', 'Togo', 'Benin', 'Senegal', 'Gambia', 'Guinea', 'Burkina Faso',  'Niger', 'Chad', 'Central African Republic','Egypt', 'Lesotho']
        for country in COUNTRIES:
            print(f"Optimizing {country}...")
            if country not in pd.read_csv(r"C:\Users\MassimoPoretti\Documents\Python Projects\Africa_Maize_Modelling\Model_physical\Results\V6_model\T_OPT_Optimization_Results.csv")['Country'].values:
                try:
                    optimize_country_temperature(country)
                except Exception as e:
                    print(f"Error optimizing {country}: {e}")
    print("\nProcessing Complete.")
