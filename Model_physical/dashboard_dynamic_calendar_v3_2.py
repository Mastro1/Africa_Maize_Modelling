...
try:
    from process_all_dynamic_calendars_v3_2 import preprocess_and_merge, refine_smoothing, USE_STSG_SMOOTHING
except ImportError:
...
RESULTS_FILE = os.path.join(BASE_DIR, "Model_physical", "Results", "Global_Dynamic_Analysis_V3_2", "Global_Calendar_V3_2.csv")
...
if os.path.exists(RESULTS_FILE):
    ...
    for col in ['SOS', 'Silking', 'EOS', 'Static_SOS', 'Static_EOS']:
...
# INITIALIZE APP
# ---------------------------------------------------------
app = dash.Dash(__name__, title="Dynamic Calendar V3.2 Analysis")

app.layout = html.Div([
    html.H1("Dynamic Calendar V3.2 Analysis Dashboard", style={'textAlign': 'center'}),
...
def generate_yearly_detail_figure(country, pcode, year):
    ...
    start_date = pd.Timestamp(year=year, month=1, day=1) - pd.Timedelta(days=60)
    end_date = pd.Timestamp(year=year, month=12, day=31) + pd.Timedelta(days=300)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=subset['date'], y=subset['NDVI_mean'], mode='lines', name='Raw NDVI', line=dict(color='#32CD32', width=1), opacity=0.6))
    ...
    for _, row in res_rows.iterrows():
        ...
        if sos_pl and eos_pl:
            ...
            fig.add_annotation(x=midpoint_pl, y=subset['NDVI_smooth'].min(), text=f"S{s_idx}: {row['Length_Days']:.0f}d | {row['GDD_Total']:.0f} GDD", showarrow=False, yshift=10, bgcolor="white", opacity=0.8)

    fig.update_layout(title=f"Calendar Verification: {pcode} ({year})", xaxis_title="Date", yaxis_title="NDVI", hovermode="x unified", template="plotly_white", height=450)
    return fig
...
if __name__ == '__main__':
    webbrowser_open = False
    if not webbrowser_open:
        import webbrowser
        webbrowser.open("http://127.0.0.1:8050/")
        webbrowser_open = True
    app.run(debug=True)
