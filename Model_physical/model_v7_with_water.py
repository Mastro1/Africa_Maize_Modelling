...
WP = 0.12             # Wilting Point (m3/m3)
FC = 0.32             # Field Capacity (m3/m3)
...
class MaizeYieldModelV7:
        ...
        for count, pcode in enumerate(pcodes, 1):
            ...
            for year in years:
                ...
                for season_idx, dates in seasons_to_process.items():
                    ...
                    # f(SM): Soil Moisture Stress (Relative Extractable Water)
                    df_season['f_SM'] = (df_season['volumetric_soil_water_layer_2'] - WP) / (FC - WP)
                    df_season['f_SM'] = df_season['f_SM'].clip(0.0, 1.0)
                    
                    # Ws: Law of the Minimum between VPD and SM stress
                    df_season['Ws'] = np.minimum(df_season['f_VPD'], df_season['f_SM'])
    ...
    def plot_example(self, df_daily, df_results, pcode):
        ...
        f_sm = ((daily_subset['volumetric_soil_water_layer_2'] - WP) / (FC - WP)).clip(0, 1)
        ws = np.minimum(f_vpd, f_sm)
        ...
        axes[1].plot(daily_subset['date'], f_sm, '--', label='SM Stress (f_SM)', alpha=0.5)
        axes[1].plot(daily_subset['date'], ws, label='Combined Water Stress (Ws)', color='black', linewidth=1.5)
...
if __name__ == "__main__":
    ...
    run_model_V7_for_country(args.country, target_pcode=args.pcode)
    print("\nProcessing Complete.")
