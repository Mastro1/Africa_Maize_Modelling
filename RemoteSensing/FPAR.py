...
def extractFPAR(country_name: str, start_date_str: str, end_date_str: str, 
    ...
    """
    Extracts mean, median, and mode of FPAR and LAI for administrative regions of a specified country 
    ...
    print(f"Starting FPAR extraction for {country_name} from {start_date_str} to {end_date_str} at Admin level {admin_level}.")
    ...
    fpar_collection = ee.ImageCollection("MODIS/061/MOD15A2H").filterDate(start_date_str, end_date_str)
    ...
    # 4. Server-side processing function (to be mapped over fpar_collection)
    def process_image_for_zonal_stats(fpar_image):
        image_date_ee = fpar_image.date()
        ...
        fpar_masked = fpar_image.select('Fpar_500m').multiply(0.01).updateMask(crop_mask)
        lai_masked = fpar_image.select('Lai_500m').multiply(0.1).updateMask(crop_mask)
        
        bands_to_reduce = fpar_masked.rename('FPAR_val').addBands(lai_masked.rename('LAI_val'))
        ...
        def sanitize_feature_and_add_date(feature_from_reduce_regions):
            ...
            f = f.set('FPAR_val_mean', feature_from_reduce_regions.get('FPAR_val_mean'))
            f = f.set('FPAR_val_median', feature_from_reduce_regions.get('FPAR_val_median'))
            f = f.set('FPAR_val_mode', feature_from_reduce_regions.get('FPAR_val_mode'))
            f = f.set('LAI_val_mean', feature_from_reduce_regions.get('LAI_val_mean'))
            f = f.set('LAI_val_median', feature_from_reduce_regions.get('LAI_val_median'))
            f = f.set('LAI_val_mode', feature_from_reduce_regions.get('LAI_val_mode'))
            
            return f

        return stats_for_image_date.map(sanitize_feature_and_add_date)

    # Map over the FPAR collection and flatten the results
    print("Mapping statistics calculation over FPAR image collection...")
    results_fc = fpar_collection.map(process_image_for_zonal_stats).flatten()
    ...
    def rename_properties(feature):
        ...
        new_props = {
            ...
            'FPAR_mean': feature.get('FPAR_val_mean'),
            'FPAR_median': feature.get('FPAR_val_median'),
            'FPAR_mode': feature.get('FPAR_val_mode'),
            'LAI_mean': feature.get('LAI_val_mean'),
            'LAI_median': feature.get('LAI_val_median'),
            'LAI_mode': feature.get('LAI_val_mode')
        }
    ...
    task = ee.batch.Export.table.toDrive(
        ...
        selectors=[
            ...
            'FPAR_mean', 'FPAR_median', 'FPAR_mode', 
            'LAI_mean', 'LAI_median', 'LAI_mode'
...
if __name__ == "__main__":
    if not EE_INITIALIZED:
        ...
        print("--- FPAR extraction sent to Earth Engine ---" )
